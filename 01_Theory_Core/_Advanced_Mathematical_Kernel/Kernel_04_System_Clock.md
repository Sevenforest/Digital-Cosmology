...
#### 1.2 Synchronization Unit (同期の最小単位)
「空間の更新」として知られていた現象を、本仕様では **「アドレッシングの拡張に伴う同期の最小単位」** と定義し直す。
システムがヒープ領域（アドレス空間）を拡張する際、全ノードの整合性を保つための最小パケット単位が、観測上の赤方偏移の最小ステップに対応する。

#### 1.3 Numerical Link to Kernel_03 ($E_{bit}$)
この量子化ステップ $\delta Z$ は、Kernel_03 2.2項で定義された **最小ビットエネルギー $E_{bit}$** に直接依存する。

$$\delta Z \propto \frac{E_{bit}}{\hbar \nu_0}$$

情報の最小単位 $E_{bit}$ が「情報の切り捨て（不感帯）」を発生させることで、基底レイテンシ $Z_{base}$ は連続的な値を取ることができず、システムクロックの倍数として段階的に更新される。
