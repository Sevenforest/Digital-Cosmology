...
#### 4.3 排熱処理：宇宙背景放射とランダウアーの原理 (Thermal Management)
情報工学における「ランダウアーの原理」によれば、情報の消去（不可逆操作）は必ず熱エネルギーの放出を伴う。
本システムにおいて、観測されなかった「未確定な計算パス」はガベージコレクション（GC）によって破棄されるが、このプロセスが物理的な「熱」を発生させる。
全宇宙で均一に観測される **「宇宙マイクロ波背景放射（CMB）」** とは、ビッグバンの名残りではなく、宇宙サーバーが常時実行しているGCプロセスに伴う **「システム排熱（CPUファンの廃熱）」** である。
